...
// Global Middlewares
// ═══════════════════════════════════════════════════════════
app.set("trust proxy", 1); // Railway/reverse proxy arkasında cookie ve protocol doğru çalışsın
app.use(
  helmet({
    crossOriginResourcePolicy: { policy: "cross-origin" }, // Farklı origin'den (Netlify) API çağrılarına izin
  }),
);
// İzin verilen origin listesi (FRONTEND_URL virgülle ayrılmış olabilir; slash'sız normalize)
...
app.use(
  cors({
    origin: (origin, callback) => {
      ...
      // Origin yoksa (same-origin, Postman, vb.) kabul et
      if (!origin) {
        return callback(null, true);
      }
      const normalized = origin.replace(/\/$/, "");
      if (allowed.includes(normalized) || allowed.includes(origin)) {
        // Credentials kullanıldığında Access-Control-Allow-Origin tam origin olmalı

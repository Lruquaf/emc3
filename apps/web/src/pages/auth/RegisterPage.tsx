...
import { registerSchema, type RegisterInput } from '@emc3/shared';
  ...
  const onSubmit = async (data: RegisterInput) => {
    ...
    try {
      ...
      if (apiError.code === 'CONFLICT') {
        setError(
          apiError.details?.field === 'email'
            ? 'Bu email adresi zaten kayıtlı'
            : 'Bu kullanıcı adı zaten alınmış'
        );
      } else {
        setError(apiError.message || 'Kayıt oluşturulamadı');
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen bg-bg flex items-center justify-center py-12 px-4">
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="mt-4 text-2xl font-semibold text-text">Hesap Oluşturun</h2>
          <p className="mt-2 text-muted">İlmî içerik dünyasına katılın</p>
        </div>
        ...
        <div className="bg-surface rounded-xl shadow-sm border border-border p-8">
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-surface text-muted">veya</span>
            </div>
          </div>
          ...
          <button
            ...
            type="button"
            className="w-full py-3 px-4 border border-border rounded-lg bg-bg text-text font-medium hover:bg-border/20 focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 transition-colors flex items-center justify-center gap-3"
          >
